...
public final class Utils {
  ...
  public static void validateKey(final byte[] key) {
    final int length = key.length;
    if (length > 250) {
      throw new IllegalArgumentException("Invalid key: key is longer than 250 bytes");
    }

    for (byte b : key) {
      if (b <= 32) {
        throw new IllegalArgumentException("Invalid key: key contains invalid bytes");
      }
    }
  }
}
